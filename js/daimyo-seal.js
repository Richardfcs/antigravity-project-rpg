 ...
 * Exportar, Importar (sobrescrever) e Mesclar (adicionar sem perder) dados.
 */

window.DaimyoSeal = (function() {
  
  /**
   * Exporta tudo (LocalStorage e IndexedDB) como JSON.
   */
  async function exportCampaign() {
    ...
    // 2. Coleta do IndexedDB (via novo método que salva keys)
    if (window.DaimyoDB) {
        try {
            await window.DaimyoDB.init();
            const stores = Object.values(window.DaimyoDB.STORES);
            for (const store of stores) {
                // getAllWithKeys resolve o problema de DBs que não usam keyPath (out-of-line keys)
                if (typeof window.DaimyoDB.getAllWithKeys === 'function') {
                    data.indexedDB[store] = await window.DaimyoDB.getAllWithKeys(store);
                } else {
                    data.indexedDB[store] = await window.DaimyoDB.getAll(store); // Fallback arriscado
                }
            }
        } catch (e) {
  ...
  /**
   * Importa um arquivo JSON e SOBRESCREVE o estado global.
   */
  async function importCampaign(file) {
    ...
    reader.onload = async function(e) {
      try {
        ...
        // Limpa estado atual (opcional, mas recomendado no overwrite)
        localStorage.clear();
            ...
            for (const store in dbData) {
                ...
                if (Array.isArray(items)) {
                    ...
                    for (const item of items) {
                        // Suporte para o novo formato {key, value} de exportação segura
  ...
  /**
   * MESCLAR (Merge) — Adiciona dados do backup SEM apagar os existentes.
   */
  async function mergeCampaign(file) {
    ...
    reader.onload = async function(e) {
      try {
        ...
        // 1. Merge LocalStorage (Apenas adiciona chaves que não existem no local)
        if (lsData) {
          for (const key in lsData) {
            if (typeof lsData[key] === 'string') {
